...
$ALLOWED = ['95.111.199.179']; 
$SALT = 'somesalt123!'; .

$VALID_USER = 'legion';
$PASS_HASH = '6b7283314ba868b11616921d42535e78'; 

$ATTEMPT_DIR = '/var/tmp/legion_attempts';
$MAX_ATTEMPTS = 5;
$LOCK_MINUTES = 15;
$IDLE_TIMEOUT = 30*60; 

if(!is_dir($ATTEMPT_DIR)) @mkdir($ATTEMPT_DIR,0700,true);
...
function ip_allowed($ip,$list){
  foreach($list as $c){
    if(strpos($c,'/')===false){ if($ip===$c) return true; continue; }
    list($sub,$bits)=explode('/',$c);
    $mask = ~((1 << (32-(int)$bits)) - 1);
    if((ip2long($ip) & $mask) === (ip2long($sub) & $mask)) return true;
  }
  return false;
}
function attempt_file($ip){ global $ATTEMPT_DIR; return $ATTEMPT_DIR.'/'.str_replace([':', '/'], '_', $ip).'.json'; }
function get_attempts($ip){ $f = attempt_file($ip); if(!is_file($f)) return ['count'=>0,'until'=>0]; $j=@json_decode(@file_get_contents($f), true); return is_array($j)?$j:['count'=>0,'until'=>0]; }
function set_attempts($ip,$d){ $f=attempt_file($ip); @file_put_contents($f,json_encode($d)); @chmod($f,0600); }


$ip = client_ip();
if(!ip_allowed($ip,$ALLOWED)){ http_response_code(403); exit("Akses ditolak: IP $ip"); }


$att = get_attempts($ip);
if(!empty($att['until']) && $att['until'] > time()){
  http_response_code(429);
  exit("Terlalu banyak percobaan. Coba lagi pada ".date('Y-m-d H:i:s',$att['until']));
}


if(isset($_GET['logout'])){ session_destroy(); header('Location: '.$_SERVER['PHP_SELF']); exit; }

function verify_login($username, $password, $salt, $valid_user, $pass_hash){
    if ($username !== $valid_user) {
        return false;
    }
    return hash('sha256', $salt . $password) === $pass_hash;
}
...
if(empty($_SESSION['ok'])){
  if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['u'], $_POST['p'])){
    ...
    if(verify_login($_POST['u'], $_POST['p'], $SALT, $VALID_USER, $PASS_HASH)){
      session_regenerate_id(true);
      $_SESSION['ok']=1;
      $_SESSION['last_active']=time();
      set_attempts($ip,['count'=>0,'until'=>0]);
      header('Location: '.$_SERVER['PHP_SELF']); exit;
    } else {
      $att['count'] = ($att['count'] ?? 0) + 1;
      if($att['count'] >= $MAX_ATTEMPTS) $att['until'] = time() + ($LOCK_MINUTES * 60);
      set_attempts($ip, $att);
      $err = 'Login salah';
    }
  }

  echo '<form method="post" style="max-width:320px">';
  if(!empty($err)) echo '<p style="color:red">'.htmlspecialchars($err).'</p>';
  echo '<input name="u" placeholder="user" required><br>';
  echo '<input name="p" type="password" placeholder="password" required><br>';
...
if(time() - $_SESSION['last_active'] > $IDLE_TIMEOUT){ session_destroy(); header('Location: '.$_SERVER['PHP_SELF']); exit; }
$_SESSION['last_active'] = time();
$Array = [
